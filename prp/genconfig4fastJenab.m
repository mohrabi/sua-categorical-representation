...
root_good = fullfile('J:\Data', "Fast", "Jenab");
root_unid = fullfile('J:\Data', "Fast", "Jenab", 'Unidentified');
root_corr = fullfile('J:\Data', "Fast", "Jenab", 'Corrupted');

sessions = [string(ls(fullfile(root_good, '20*')));
    string(ls(fullfile(root_unid, '20*')));
...
sessionInfo(ismember(sessions, string(ls(fullfile(root_unid, '20*')))), 2) = "unidentified";
sessionInfo(ismember(sessions, string(ls(fullfile(root_corr, '20*')))), 2) = "corrupted";

recordingInfo = readmatrix(fullfile('G:\Data', "Fast", "Jenab", "recordingInfo.xlsx"), ...
...
writematrix(sessionInfo, 'G:\Data\Fast\Jenab\sessionInfo.csv');
...
sessionInfo = readmatrix('G:\Data\Fast\Jenab\sessionInfo.csv', ...
    'FileType', 'spreadsheet', ...
    'NumHeaderLines', 0, ...
    'OutputType', 'string');
...
for iRow = 1:size(info, 1)
    root = fullfile('G:\Data\Fast\Jenab', info(iRow, 1), 'Trial');
...
writematrix(itcNeuralInfo, 'G:\Data\Fast\Jenab\itcNeuralInfo.csv');
save('G:\Data\Fast\Jenab\itcNeuralInfo.mat', 'itcNeuralInfo');
writematrix(pfcNeuralInfo, 'G:\Data\Fast\Jenab\pfcNeuralInfo.csv');
save('G:\Data\Fast\Jenab\pfcNeuralInfo.mat', 'pfcNeuralInfo');
