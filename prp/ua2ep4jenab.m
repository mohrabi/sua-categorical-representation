...
monkey_dir = 'J:\Data\Fast\Jenab';
export_dir = 'G:\Data\Fast\Jenab';
...
for isession = 1
    ...
    out_dir = fullfile(export_dir, sessions(isession, :));
    rmdir(out_dir, 's')
    mkdir(out_dir)
    ...
    % Load IT Trials
    if isfolder(cln_dir) && ~isfile(fullfile(cln_dir, '__IT_NO_LOAD__'))
        ...
        status(isession, 1) = 0;
    elseif ~isfolder(cln_dir)
        [it_spike_times, it_optimal_set] = load_sorted_spikes(fullfile(mat_dir, 'spk_it_sor.mat'));
        it_epochs = create_data_epochs_from_mu(it_spike_times, it_optimal_set, trial_time);
        save_epochs(it_epochs, out_dir, 'it')
        status(isession, 1) = 1;
    end
    
    % Load PFC Trials
    if isfolder(cln_dir) && ~isfile(fullfile(cln_dir, '__PFC_NO_LOAD__')) && ...
            isfile(fullfile(cln_dir, 'spk_pfc_sor.mat'))
        [it_spike_times, it_optimal_set] = load_sorted_spikes(fullfile(cln_dir, 'pfc.mat'));
        it_epochs = create_data_epochs(it_spike_times, it_optimal_set, trial_time);
        save_epochs(it_epochs, out_dir, 'pfc')
        status(isession, 2) = 0;
    elseif ~isfolder(cln_dir)
        [it_spike_times, it_optimal_set] = load_sorted_spikes(fullfile(mat_dir, 'spk_pfc_sor.mat'));
        it_epochs = create_data_epochs_from_mu(it_spike_times, it_optimal_set, trial_time);
        save_epochs(it_epochs, out_dir, 'pfc')
        status(isession, 2) = 1;
    end
...
monkey_dir = 'J:\Data\Fast\Jenab\Unidentified';
export_dir = 'G:\Data\Fast\Jenab';
sessions = ls(fullfile(monkey_dir, '20*'));
nsession = size(sessions, 1);
